...
public class ProjectionNewView extends JPanel{

	/**
	 * 
	 */
	private static final long serialVersionUID = -301549675217550580L;
	...
	public ProjectionNewView(ControleurProjections ce, final java.util.List<String> titresFilms, final java.util.List<String> nosSalles) {
		...
		JLabel vueTitre = new JLabel("<html><body><br/><h2 style=\"margin-left: 10px; font-style:normal; color:black\">"+"Nouvelle projection " +"</h2></body></html>");
		...
		acceptButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				try {
					...
					controleurProjections.newProjection(dateChoisie, titreFilm, noSalle);
				}
		...
		clearButton.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				controleurProjections.cancelCreation();
			}
		});
	...
	public void setInfo(){
		Calendar date = Calendar.getInstance();		// date actuelle
		dateModel.setDate(date.get(Calendar.YEAR), date.get(Calendar.MONTH), date.get(Calendar.DAY_OF_MONTH));
